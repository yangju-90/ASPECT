...
master_output_stats.to_csv(r'../Output/county_output/aggregated_stats.csv' )


############################# dasymetric mapping #############################
print('#############################starting the regression#############################')
perform = r'../Output/county_output/performance.csv'
p_df = pd.DataFrame() # define a dataframe to save best model parameters and performances
master_output_stats = pd.read_csv(r'../Output/county_output/aggregated_stats.csv' )
...
for pvar in tpop_dict.keys():
    ...
    for c in cities:
        print(c)
        files = glob.glob(r'..\Processing\county\%s\masked_final_*.tif' % (c))
        idx = 0
        shape = rasterio.open(files[0]).shape
        predictors = np.empty(shape, dtype=rasterio.float64)
        for v in vars:
            fname = r'..\Processing\county\%s\masked_final_%s.tif' % (c, v.split('_')[1].split('.')[0])
        ...
        kwargs.update(
            ...
            nodata=-999,
            count=1)
        out_res = r'../Processing/county/weight_%s_%s.tif' % (pvar,c)
        ...
        out_pop = '../Output/county_output/%s_%s.tif' % (pvar,c)
